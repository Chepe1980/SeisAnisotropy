...
def main_processing(df, vp_col='VP', vs_col='VS', rho_col='RHOB', vclay_col='VCLAY', phi_col='PHIT', method='default'):
    ...
    # Estimate Thomsen parameters for entire well
    ...
    # Calculate elastic constants for entire well
    constants = calculate_elastic_constants(vp, vs, rho, epsilon, gamma, delta)
    
    for key, value in constants.items():
        result_df[key] = value
    
    # Calculate A, B, C attributes for entire well
    A = rho * vp
...
def plot_angle_gather(angle_axis, depth_axis, synthetic_gather, title, colormap='RdBu'):
    ...
    fig.update_layout(
        ...
        yaxis_title='Depth (m)',
        yaxis=dict(autorrange="reversed"),
        width=800,
        height=600
    )
    return fig
...
def main():
    ...
    with tab1:
        ...
        # Process the data - Thomsen parameters calculated for entire well
        ...
        # Get properties for all three layers (using pre-calculated Thomsen parameters from entire well)
        layer_properties = []
        ...
        for i, col in enumerate(cols):
            if i < len(layer_properties):
                with col:
                    ...
                    if 'epsilon' in layer_properties[i]:
                        st.metric("Epsilon (ε)", f"{layer_properties[i]['epsilon']:.4f}")
                    if 'gamma' in layer_properties[i]:
                        st.metric("Gamma (γ)", f"{layer_properties[i]['gamma']:.4f}")
                    if 'delta' in layer_properties[i]:
                        st.metric("Delta (δ)", f"{layer_properties[i]['delta']:.4f}")
        ...
        if len(layer_properties) >= 3:
            ...
            # Get attribute ratios for interface 1 (using pre-calculated values from entire well)
            ...
            # Get attribute ratios for interface 2 (using pre-calculated values from entire well)
    ...
    with tab2:
        ...
        if 'result_df' in locals():
            # Display Thomsen parameters for entire well
            st.subheader("Thomsen Anisotropy Parameters (Entire Well)")
            ...
            st.subheader("Elastic Constants (Entire Well)")
            
            elastic_fig = make_subplots(rows=1, cols=5, 
                                      subplot_titles=('C11', 'C13', 'C33', 'C44', 'C66'),
                                      shared_yaxes=True)
            
            elastic_constants = ['c11', 'c13', 'c33', 'c44', 'c66']
            colors = ['blue', 'green', 'red', 'orange', 'purple']
            
            for i, constant in enumerate(elastic_constants):
                if constant in result_df.columns:
                    elastic_fig.add_trace(go.Scatter(
                        x=result_df[constant], 
                        y=result_df[depth_col], 
                        mode='lines',
                        name=constant,
                        line=dict(color=colors[i], width=1)
                    ), row=1, col=i+1)
            
            elastic_fig.update_yaxes(title_text="Depth (m)", autorange="reversed", row=1, col=1)
            for i in range(1, 6):
                elastic_fig.update_xaxes(title_text=f"c{i}{i if i < 4 else i+2}" if i != 2 else "c13", row=1, col=i)
